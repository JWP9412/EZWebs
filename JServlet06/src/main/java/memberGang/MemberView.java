...
public class MemberView extends HttpServlet {
	...
	static void outHtmlMembers(PrintWriter out, List<MemberVO> members) {
		...
		for(int cnt=0; cnt < members.size(); cnt++) {
			...
			out.print("<tr>");
			out.printf("<td><a href='/JServlet06/memberdetailG?mid=%s'>%s</a></td>", member.getMid(), member.getMid());
			...
			out.printf("<td><a href='/JServlet06/memberlistG?command=delMember&mid=%s'>삭제</a></td>", member.getMid());
			out.printf("<td><a href='/JServlet06/membereditG?mid=%s'>수정</a></td>", member.getMid());
			out.print("</tr>");
		}
	}
